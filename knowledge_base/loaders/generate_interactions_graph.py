...
<body>
    ...
    <section class="canvas-panel">
      <div class="toolbar">
        <div style="display:flex; gap:10px; align-items:center;">
          ...
          <label>Search
            <input id="search" placeholder="type class name" style="margin-left:6px; padding:5px 8px; border:1px solid #cbd5e1; border-radius:4px;" />
          </label>
          <button id="btnZoomIn" title="Zoom in">＋</button>
          <button id="btnZoomOut" title="Zoom out">−</button>
          <button id="btnReset" title="Reset view">Reset</button>
          <button id="btnExport" title="Export PNG">Export PNG</button>
        </div>
        <div class="legend" style="margin-left:12px;">
          <label style="font-weight:600; margin-right:8px;">Severity:</label>
          <label><input type="checkbox" class="sev-filter" data-sev="CONTRAINDICATED" checked /> <span class="contraindicated">Contraindicated</span></label>
          <label><input type="checkbox" class="sev-filter" data-sev="MAJOR" checked /> <span class="major">Major</span></label>
          <label><input type="checkbox" class="sev-filter" data-sev="MODERATE" checked /> <span class="moderate">Moderate</span></label>
          <label><input type="checkbox" class="sev-filter" data-sev="MINOR" checked /> <span class="minor">Minor</span></label>
          <label><input type="checkbox" class="sev-filter" data-sev="UNKNOWN" checked /> <span class="unknown">Unknown</span></label>
        </div>
      </div>
      <svg viewBox="0 0 1000 820" role="img" aria-label="Drug class interaction graph">
        <g id="viewport"></g>
      </svg>
    </section>
  ...
  <script>
    ...
    function render(limit) {{
      ...
      line.setAttribute('data-severity', e.severity);
      ...
      circle.setAttribute('data-severity', n.severity);
    ...
    svgNode.addEventListener('wheel', e => {{
      ...
      scale = Math.max(0.2, Math.min(6, scale * zoomFactor));
      // optional: adjust tx/ty to zoom toward mouse position (approximate)
    ...
    svgNode.addEventListener('mousedown', e => {{ isPanning = true; panStart = {{x: e.clientX, y: e.clientY}}; }});
    window.addEventListener('mousemove', e => {{
      if (!isPanning) return;
      const dx = e.clientX - panStart.x; const dy = e.clientY - panStart.y;
      panStart = {{x: e.clientX, y: e.clientY}};
      tx += dx; ty += dy; setTransform();
    }});
    window.addEventListener('mouseup', () => {{ isPanning = false; panStart = null; }});

    document.getElementById('btnZoomIn').addEventListener('click', () => {{ scale = Math.min(6, scale * 1.2); setTransform(); }});
    document.getElementById('btnZoomOut').addEventListener('click', () => {{ scale = Math.max(0.2, scale / 1.2); setTransform(); }});
    document.getElementById('btnReset').addEventListener('click', () => {{ scale = 1; tx = 0; ty = 0; setTransform(); }});

    // export PNG
    document.getElementById('btnExport').addEventListener('click', async () => {{
      const clone = svgNode.cloneNode(true);
      // apply current transform onto the cloned viewport group
      const gv = clone.querySelector('#viewport');
      if (gv) gv.setAttribute('transform', `translate(${{tx}},${{ty}}) scale(${{scale}})`);
      ...
      img.onload = () => {{
        ...
        canvas.width = clone.viewBox.baseVal.width || 1000;
        canvas.height = clone.viewBox.baseVal.height || 820;
        const ctx = canvas.getContext('2d');
        ctx.fillStyle = '#ffffff'; ctx.fillRect(0,0,canvas.width,canvas.height);
        ctx.drawImage(img,0,0);
        URL.revokeObjectURL(url);
        const png = canvas.toDataURL('image/png');
        const a = document.createElement('a'); a.href = png; a.download = 'interactions_graph.png'; a.click();
      }};
      img.src = url;
    }});
    ...
    search.addEventListener('keyup', (e) => {{
      ...
      if (!q) return;
      const node = graph.nodes.find(n => n.id.toLowerCase().includes(q));
    ...
    function centerOn(node) {{
      ...
      tx = cx - node.x * scale; ty = cy - node.y * scale; setTransform();
      highlight(node.id);
    }}

    // severity filters
    document.querySelectorAll('.sev-filter').forEach(cb => cb.addEventListener('change', () => {{
      const enabled = Array.from(document.querySelectorAll('.sev-filter')).filter(x => x.checked).map(x => x.dataset.sev);
      for (const el of svgNode.querySelectorAll('.node')) {{
        el.style.display = enabled.includes(el.getAttribute('data-severity')) ? '' : 'none';
      }}
      for (const el of svgNode.querySelectorAll('.edge')) {{
        el.style.display = enabled.includes(el.getAttribute('data-severity')) ? '' : 'none';
      }}
    }}));
...
def main():
    ...
    input_csv = base / "sources" / "dataset" / "interactions_grouped_by_class.csv"
    out_dir = base / "graph"
